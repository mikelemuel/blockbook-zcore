...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "zcore.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "zcored.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ZCRand zZCRmoney supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ZCR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ZCR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, zcore, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ZCR/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ZCore specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcrstake=<n>", strprintf(_("Enable or disable staking functionality for ZCRinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zzcrstake=<n>", strprintf(_("Enable or disable staking functionality for zZCRinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzzcr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zZCRminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zzcrbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zZCRbackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizezcoreamount=<n>     " + strprintf(_("Keep N ZCRanonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The ZCore Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ZCore is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ZCore is shutting down."));
...
#endif
    // Make sure only a single ZCore process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZCore is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //ZCore: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ZCore specific: zerocoin and spork DB's
                ...
                // ZCore: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZZCRMinted();
                        RecalculateZZCRSpent();
                    }
                    RecalculateZCRSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // ZCore: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ZCore") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ZCore to complete") << "\n";
        ...
        zwalletMain = new CzZCRWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zZCRWallet
        uiInterface.InitMessage(_("Syncing zZCRwallet..."));

        bool fEnableZZcrBackups = GetBoolArg("-backupzzcr", true);
        pwalletMain->setZZcrAutoBackups(fEnableZZcrBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zzcrTracker->Init();
...
//    }
//
//    nAnonymizeZCoreAmount = GetArg("-anonymizezcoreamount", 0);
//    if (nAnonymizeZCoreAmount > 999999) nAnonymizeZCoreAmount = 999999;
//    if (nAnonymizeZCoreAmount < 2) nAnonymizeZCoreAmount = 2;
    ...
    LogPrintf("Anonymize ZCore Amount %d\n", nAnonymizeZCoreAmount);
    ...
    /* Denominations
       ...
       For example:
       1ZCR+1000 == (.1ZCR+100)*10
       10ZCR+10000 == (1ZCR+1000)*10
    */
