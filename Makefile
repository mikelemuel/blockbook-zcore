...
pkgdatadir = $(datadir)/zcore
pkgincludedir = $(includedir)/zcore
pkglibdir = $(libdir)/zcore
pkglibexecdir = $(libexecdir)/zcore
...
CONFIG_HEADER = $(top_builddir)/src/config/zcore-config.h
...
ACLOCAL = ${SHELL} /root/test/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/test/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/test/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/test/build-aux/missing automake-1.15
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = zcore-cli
BITCOIN_DAEMON_NAME = zcored
BITCOIN_GUI_NAME = zcore-qt
BITCOIN_TX_NAME = zcore-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/test/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/test
abs_srcdir = /root/test
abs_top_builddir = /root/test
abs_top_srcdir = /root/test
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/test/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_zcore_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_zcore_coverage.info test_zcore.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/ZCore-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/zcore-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/zcore-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/zcore-config.h
$(top_srcdir)/src/config/zcore-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/zcore-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/ZCore-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZCore-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZCore-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_zcore.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_zcore -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_zcore_filtered.info: test_zcore.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_zcore_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_zcore_coverage.info: baseline_filtered_combined.info test_zcore_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zcore_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_zcore_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zcore_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_zcore.coverage/.dirstamp:  test_zcore_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_zcore.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_zcore.coverage/ total.coverage/ $(OSX_APP)
